...
import { galleryCategoryPath, type GalleryCategorySlug } from '../../data/gallery';
...
export function ImageCard({ image, imageSmall, imageMedium, width, height, title, text, galleryCategory, index = 0 }: ImageCardProps) {
  const { t } = useLanguage();
  ...
  return <article className="image-card">
    ...
    <div className="image-card__body"><h3>{t(title)}</h3><p>{t(text)}</p><Link to={galleryCategoryPath(galleryCategory)}>{t('View')} {t(title)} {t('tattoo portfolio')} <span aria-hidden="true">→</span></Link></div>
  </article>;
}
