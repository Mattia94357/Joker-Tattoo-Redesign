...
export function MobileMenu({ open, close }: { open: boolean; close: () => void }) {
  ...
  return <AnimatePresence>{open && <m.div className="mobile-menu" role="dialog" aria-modal="true" aria-label={t('Navigate the studio')} initial={{ clipPath: 'circle(0% at 90% 5%)' }} animate={{ clipPath: 'circle(150% at 90% 5%)' }} exit={{ clipPath: 'circle(0% at 90% 5%)' }} transition={{ duration: .55, ease: [0.76, 0, 0.24, 1] }}>
    ...
    <button className="button mobile-menu__booking" onClick={() => { close(); openBooking(); }}>{t('Book a Tattoo')}<span aria-hidden="true">↗</span></button>
